...
| Fully unsupervised CV output without a labeled validation set | Growth-stage/harvest-readiness/disease classification are inherently ambiguous without ground truth; every CV metric requires a labeled dataset before being considered done (CV-02) |
| Growth-stage classification, harvest-readiness signal, disease/anomaly detection (CV-03/CV-04/CV-05) | Deferred to v2 — moved out of Phase 5 MVP scope: 4-stage classification + per-class disease detection (≥100 labeled images/class × 4 classes) needs a labeling effort too large for this milestone's timeline; ship size/count/coverage measurement first |
...
*Last updated: 2026-08-27 — cut Phase 5 CV scope to MVP: moved CV-03/CV-04/CV-05 (stage classification, harvest-readiness, disease detection) from v1 to v2*
