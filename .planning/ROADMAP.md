...
**Goal**: Operators can see quantified mushroom growth (size, count, coverage area) derived from scheduled camera captures — a pipeline architecturally decoupled from the actuator control loop so it can never degrade environmental safety. Growth-stage classification, harvest-readiness, and disease/anomaly detection deferred to v2 (require larger labeled-dataset effort than this milestone's timeline supports).
**Mode:** mvp
**Depends on**: Phase 1
**Requirements**: CV-01, CV-02, FAIL-04
**Success Criteria** (what must be TRUE):
